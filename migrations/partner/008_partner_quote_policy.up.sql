...
BEGIN
  ...
  IF snapshot_value#>>'{contract,adapterMode}' IS DISTINCT FROM company_row.submission_adapter_mode
    ...
    OR snapshot_value#>'{job,leadSources}' IS DISTINCT FROM jsonb_build_array(normalize(btrim(company_row.name),NFC)) OR snapshot_value#>>'{job,notes}' IS DISTINCT FROM normalize(job_row.notes,NFC)
    OR snapshot_value#>'{job,quote}' IS DISTINCT FROM public.partner_submission_jsonb_nfc(job_row.quote_data || '{"consentFeeCents":0,"depositBasisPoints":0}'::jsonb)
  ...
  UPDATE public.partner_jobs SET quote_data=snapshot_value#>'{job,quote}',
    quote_total_cents=GREATEST(0,quote_total_cents-COALESCE((job_row.quote_data->>'consentFeeCents')::bigint,0)), submission_state='QUEUED',submission_checkpoint='FROZEN',submission_adapter_mode_snapshot=company_row.submission_adapter_mode,
