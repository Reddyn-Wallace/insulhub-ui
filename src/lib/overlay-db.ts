...
async function ensureOverlaySchemaInternal() {
  ...
  await overlaySql`CREATE TABLE IF NOT EXISTS job_sms_messages (
  id uuid PRIMARY KEY,
  insulhub_job_id text NOT NULL,
  job_number integer,
  ...
  await overlaySql`ALTER TABLE job_sms_messages DROP CONSTRAINT IF EXISTS job_sms_messages_sender_id_fkey`;
  await overlaySql`CREATE INDEX IF NOT EXISTS job_sms_messages_job_created_idx ON job_sms_messages(insulhub_job_id, created_at DESC)`;
