...
export async function jobSmsIdentity(request: NextRequest, jobId?: string) {
  ...
  const response = await fetch("https://api.insulhub.nz/graphql", {
    ...
    body: JSON.stringify({ query: jobId
      ? "query JobSmsAccess($id:ObjectId!){me{_id firstname lastname role} job(_id:$id){_id jobNumber client{contactDetails{name email phoneMobile phoneSecondary}}}}"
  ...
  return json.data as { me: { _id: string; firstname: string; lastname: string; role: string }; job?: { _id: string; jobNumber: number; client?: { contactDetails?: { name?: string; email?: string; phoneMobile?: string; phoneSecondary?: string } } } };
}
