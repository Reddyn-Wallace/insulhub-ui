...
export async function GET(request: NextRequest, context: Context) {
  try {
    ...
    const senders = await overlaySql`SELECT id,label,sender_value,provider_config FROM communication_senders WHERE owner_user_id=${me._id} AND channel='email' AND provider='gmail' AND is_active=true AND connection_status='connected' ORDER BY is_default DESC,label`;
    return NextResponse.json({ enabled: true, senders: senders.map(sender => ({ id: sender.id, label: sender.label, senderValue: sender.sender_value,
