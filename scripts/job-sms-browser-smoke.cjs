...
(async () => {
  ...
  try {
    for (const width of [390, 1280]) {
      ...
      await page.getByText('Accepted by SMS service',{exact:true}).first().waitFor();
      assert.equal(sends,1);assert.equal(message.body,'Exact test message');
      await page.getByRole('button',{name:'Check message status',exact:true}).click();
      await page.getByRole('status').getByText('Sent',{exact:true}).waitFor();
      ...
      await page.reload();
      await page.getByRole('button',{name:'Send SMS from CRM',exact:true}).click();
      assert.equal(await page.getByRole('textbox',{name:'Message',exact:true}).inputValue(),'Exact test message');
